...
export default function MentorSignUpPage() {
  ...
  useEffect(() => {
    ...
    getCurrentUser().then((currentUser) => {
      if (currentUser) { navigate("/dashboard_mentor"); return; }
      if (errorParam) {
        try {
  ...
  const handleSignUp = async (e) => {
    ...
    setError("");
    try {
      await signup(name, email, password, 'mentor', verifiedPhone);
  ...
  const handleGoogleSignUp = async () => {
    try {
      await loginWithGoogle(navigate, true, 'mentor', verifiedPhone);
    } catch (err) {
        ...
        <div className="w-full lg:w-1/2 bg-white flex flex-col justify-center px-8 py-12 lg:px-16 relative">
          ...
          <div className="max-w-md mx-auto w-full mt-8">
            ...
            {step === 'phone' ? (
              <>
                <div className="mb-6">
                  <h2 className="text-3xl font-semibold text-blue-600 mb-1">Become a Mentor</h2>
                  <p className="text-gray-600">First, let's verify your phone number</p>
                </div>
                <PhoneOtpGate userType="mentor" onVerified={handlePhoneVerified} />
                <div className="mt-6 text-center text-gray-600 text-sm">
                  Already a mentor?{" "}
                  <button onClick={() => navigate("/mentor-login")} className="text-blue-600 hover:underline font-medium">Log in</button>
                </div>
                <div className="mt-3 text-center text-gray-500 text-sm">
                  Looking for a mentor instead?{" "}
                  <button onClick={() => navigate("/signup")} className="text-blue-600 hover:underline font-medium">Sign up as Mentee</button>
                </div>
              </>
            ) : (
              <>
                <div className="mb-6">
                  <h2 className="text-3xl font-semibold text-blue-600 mb-1">Create Mentor Account</h2>
        ...
        <div className="hidden lg:flex lg:w-1/2 justify-center items-center bg-gradient-to-br from-gray-50 to-gray-100 p-4">
          <div className="w-full h-full overflow-hidden rounded-2xl shadow-xl bg-white p-2">
            <div className="rounded-xl overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-50 p-8 h-full flex items-center justify-center">
              <img src={SignupImage} alt="Mentor Sign Up" className="rounded-xl shadow-lg w-full h-auto object-cover" />
            </div>
          </div>
        </div>
      </div>
