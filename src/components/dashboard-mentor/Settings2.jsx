...
import { Settings as SettingsIcon, User, Bell, Target, BookOpen, Users, Shield, HelpCircle, ExternalLink, Globe, Calendar, MapPin, Briefcase, Star, Mail, Phone, Map, Linkedin, Link as LinkIcon, Edit2, Download, FileText, Menu, ChevronDown, Check, CheckCircle, XCircle, Clock, Slash, Plus } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 lg:gap-8 animate-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/20 border border-slate-100 relative overflow-hidden group">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8 relative z-10">
            ...
            <div className="relative group flex-shrink-0">
              <div className="w-28 h-28 lg:w-32 lg:h-32 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-[2rem] flex items-center justify-center text-white text-4xl font-bold shadow-2xl shadow-blue-500/30 transform group-hover:scale-105 transition-all duration-300">
                {getInitials(user?.name || user?.email)}
              </div>
              <button className="absolute -bottom-2 -right-2 bg-white p-3 rounded-2xl shadow-lg border border-slate-100 text-blue-600 hover:text-blue-700 hover:scale-110 transition-all">
            ...
            <div className="flex-1 text-center md:text-left w-full min-w-0 space-y-4">
              <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-4">
                <div>
                  ...
                  <p className="text-slate-500 font-medium text-lg">Software Engineer</p>
                </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/20 border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="col-span-full">
              <label className="text-sm font-bold text-slate-900 mb-3 block ml-1">Professional Bio</label>
              <textarea
      ...
      <div className="xl:col-span-4 space-y-6">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/20 border border-slate-100 relative z-20" ref={statusRef}>
          <h3 className="font-bold text-slate-900 mb-6 text-xl">Availability Status</h3>
          <div
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/20 border border-slate-100">
          ...
          <div className="flex flex-wrap gap-2">
            {['React Native', 'System Design', 'JavaScript', 'Leadership', 'Mentoring', 'Python', 'Cloud Arch'].map((skill, index) => (
              <div
                key={index}
                className="group flex items-center gap-2 bg-slate-50 hover:bg-blue-50 text-slate-600 hover:text-blue-600 px-4 py-2.5 rounded-xl text-xs font-bold border border-slate-100 hover:border-blue-200 transition-all duration-200 cursor-default"
              >
                <span>{skill}</span>
              </div>
            ))}
            <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold text-slate-400 border border-dashed border-slate-300 hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all duration-200">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-dm relative overflow-x-hidden">
      ...
      <div className="relative z-10">
        <Navigation />
        <Sidenav sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} currentRoute="/dashboard-mentor/settings" />
